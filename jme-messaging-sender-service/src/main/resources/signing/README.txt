...
###################
# Create Leaf (End-Entity) Certificate for jme-messaging-sender-service

# Generate the Leaf Private Key
openssl genpkey -algorithm RSA -out jme-messaging-sender-service.key -pkeyopt rsa_keygen_bits:2048

# Create the Leaf Certificate Signing Request (CSR)
openssl req -new -key jme-messaging-sender-service.key -out jme-messaging-sender-service.csr -subj "/C=CH/ST=Bern/L=Bern/O=BIT/OU=jeap/CN=jme-messaging-sender-service"

# Sign the Leaf Certificate with the Intermediate CA
openssl x509 -req -in jme-messaging-sender-service.csr -CA intermediateCA.crt -CAkey intermediateCA.key -CAcreateserial -out jme-messaging-sender-service.crt -days 3000 -sha256 -passin pass:strongpassword

####################
AWS how to set the secrets
####################

aws secretsmanager get-secret-value --secret-id jme-messaging-sender-service --query SecretString --output text > current_secret.json


NEW_VALUE_KEY=$(cat jme-messaging-sender-service.key)
...
aws secretsmanager update-secret --secret-id jme-messaging-sender-service --secret-string file://updated_secret.json
