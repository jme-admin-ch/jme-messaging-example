# NOTE CA and Intermediate CA are generated once, the leaf certificate can be generated multiple times
###################
...
###################
# Create Leaf (End-Entity) Certificate for jme-messaging-receiverpublisher-service
...
aws secretsmanager get-secret-value --secret-id jme-messaging-receiverpublisher-service --query SecretString --output text > current_secret.json

NEW_VALUE_KEY=$(cat jme-messaging-receiverpublisher-service.key)
...
aws secretsmanager update-secret --secret-id jme-messaging-receiverpublisher-service --secret-string file://updated_secret.json
